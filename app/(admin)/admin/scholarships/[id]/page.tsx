...
export default function EditScholarshipPage() {
		...
		const fetchData = async () => {
			try {
				...
				setApplicationOpenDate(
					scholarship.applicationOpenDate?.split("T")[0] || "",
				);
				...
				setError(
					err instanceof Error ? err.message : "Failed to load scholarship",
				);
			} finally {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to update scholarship",
			);
		} finally {
			setIsLoading(false);
		}
